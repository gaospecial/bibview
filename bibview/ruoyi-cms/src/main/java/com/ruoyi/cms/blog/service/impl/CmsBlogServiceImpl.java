...
public class CmsBlogServiceImpl implements ICmsBlogService 
{
    ...
    @Resource
    private CmsBlogBibServiceImpl cmsBlogBibService;
    ...
    public CmsBlog selectCmsBlogById(Long id)
    {
        ...
        //查询文献列表
        CmsBlogBib cmsBlogBib=new CmsBlogBib();
        cmsBlogBib.setBlogId(id);
        List<CmsBlogBib> cmsBlogBibList = cmsBlogBibService.selectCmsBlogBibList(cmsBlogBib);
        System.out.println(cmsBlogBibList);

        Long[] bibIds = new Long[cmsBlogBibList.size()];
        for (int i = 0; i<cmsBlogBibList.size();i++){
            CmsBlogBib cBlogBib = cmsBlogBibList.get(i);
            Long bibId = cBlogBib.getBibId();
            bibIds[i] = bibId;
            cBlogBib.setBibId(bibId);
            Bib bib=bibMapper.selectBibById(bibId);
            bibs.add(bib);
        }
        blog.setBibId(bibIds);
        blog.setBibs(bibs);
    ...
    public Long insertCmsBlog(CmsBlog cmsBlog)
    {
        ...
        //新增文章文献
        Long[] bibId=cmsBlog.getBibId();
        if (bibId!=null&&bibId.length>0){
            for (Long bibId1 : bibId) {
                CmsBlogBib cmsBlogBib = new CmsBlogBib();
                cmsBlogBib.setBlogId(blogId);
                cmsBlogBib.setBibId(bibId1);
                cmsBlogBibService.insertCmsBlogBib(cmsBlogBib);
            }
        }
    ...
    public int updateCmsBlog(CmsBlog cmsBlog)
    {
        ...
        //清空文章文献关联
        cmsBlogBibService.deleteCmsBlogBibByBlogId(blogId);
        ...
        Long[] bibId=cmsBlog.getBibId();
        if (bibId!=null&&bibId.length>0){
            List<CmsBlogBib> blogBibs=new ArrayList<>();
            for (Long bibId1 : bibId) {
                CmsBlogBib cmsBlogBib = new CmsBlogBib();
                cmsBlogBib.setBlogId(blogId);
                cmsBlogBib.setBibId(bibId1);
                blogBibs.add(cmsBlogBib);
            }
            cmsBlogBibService.batchBlogBib(blogBibs);
        }
