        ...
        <el-row>
          <el-col :span="7">
            <el-form-item label="文献">
              <el-select v-model="form.bibId" multiple placeholder="请选择" filterable clearable>
                <el-option v-for="item in zoteros" :label="item.title" :key="item.id"  :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
          </el-col>
...
<script>
  ...
  import { listBib  } from "@/api/system/bib";
  import {
    listBlog,
    getBlog,
    delBlog,
    ...
    methods: {
      ...
      handleAdd() {
        getBlog().then(response => {
          ...
          this.bibOptions=response.bibs;
          this.reset();
          this.open = true;
          this.title = "添加文章";
        });

        // listBib().then(response =>{
        //   this.bibOptions =response.rows
        // })
      },
      /** 修改按钮操作 */
      handleUpdate(row) {
