    ...
    <el-col :xs="24" :sm="18">
      <el-card style="background-color: rgba(255, 255, 255,1)" class="first-card">
        ...
        <div class="bib-list-container">
          <div class="bib-list">
            <div v-for="(bib, index) in blog.bibs" :key="index" class="bib-item">
              <div class="bib-item-container">
                <h3>{{ bib.title }}</h3>
                <p>{{ bib.abstractNote }}</p>
              </div>
              <button @click="showItemPopup(bib)" class="more-button">更多</button>
                <!--    加个横线用于区分          -->
        ...
        <!--    文献详情    -->
        <div class="bib-detail-container">
          <div class="bib-detail">
            <div class="bib-detail-header">
              <h2>{{ selectedBib.title }}</h2>
              <button @click="hideItemPopup" class="close-button">关闭</button>
            </div>
            <div class="bib-detail-content">
              <p>{{ selectedBib.abstractNote }}</p>
              <p>{{ selectedBib.year }}</p>
              <p>{{ selectedBib.authors }}</p>
              <p>{{ selectedBib.publicationTitle }}</p>
              <p>{{ selectedBib.address }}</p>
              <p>{{ selectedBib.pages }}</p>
              <p>{{ selectedBib.doi }}</p>
              <p>{{ selectedBib.url }}</p>
              <p>{{ selectedBib.callNumber }}</p>
              <p>{{ selectedBib.extra }}</p>
              <p>{{ selectedBib.note }}</p>
              <p>{{ selectedBib.accessDate }}</p>
              <p>{{ selectedBib.libraryCatalog }}</p>
              <p>{{ selectedBib.shortTitle }}</p>
              <p>{{ selectedBib.archive }}</p>
              <p>{{ selectedBib.archiveLocation }}</p>
              <p>{{ selectedBib.language }}</p>
              <p>{{ selectedBib.rights }}</p>
              <p>{{ selectedBib.tags }}</p>
              <p>{{ selectedBib.series }}</p>
              <p>{{ selectedBib.seriesNumber }}</p>
              <p>{{ selectedBib.ISBN }}</p>
              <p>{{ selectedBib.ISSN }}</p>
              <p>{{ selectedBib.publisher }}</p>
              <p>{{ selectedBib.pmid }}</p>
              <p>{{ selectedBib.pmcid }}</p>
              <p>{{ selectedBib.abstract }}</p>
              <p>{{ selectedBib.url }}</p>
              <p>{{ selectedBib.accessDate }}</p>
              <p>{{ selectedBib.archive }}</p>
              <p>{{ selectedBib.archiveLocation }}</p>
...
<style scoped>
  ...
  .bib-list-container {
    ...
    padding: 10px; /* 添加内边距 */
    margin-bottom: 20px; /* 与其他内容隔开 */
  }

  .bib-item {
    margin-bottom: 15px; /* 各个文献项目之间留出一些间距 */
  }

  .more-button {
    margin-top: 10px; /* 与文献列表之间留出一些间距 */
  }
